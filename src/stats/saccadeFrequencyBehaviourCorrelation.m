...
for ll = 1:numel(windowSize)
    ...
    for ii = 1:numel(subject)
        for j = 1:numel(trialNum)
            ...
            fileName1 = [preFolder, cell2mat(subject(ii)),'\',num2str(trialNum(j)),'\','saccadicFrequency_window',num2str(windowSize(ll)),'.csv'];
            if isfile(fileName1)
                ...
                if (numel(saccadicFrequency) < numel(avgSpeed))
                    avgSpeed = avgSpeed(1:numel(saccadicFrequency));
                    avgTurnRate = avgTurnRate(1:numel(saccadicFrequency));
                end
                if (numel(saccadicFrequency) > numel(avgSpeed))
                    saccadicFrequency = saccadicFrequency(1:numel(avgSpeed));
                end
            end
        ...
        c2 = c2 + 1;
        if (iscolumn(saccadicFrequency))
            saccadicFrequency = saccadicFrequency';
        end
    ...
    stdSaccadeFreq = std(aggSaccadeFreq);
    meanSaccadeFreq = mean(aggSaccadeFreq);

    % indRej = abs(aggSaccadeFreq) < 4 ;
    ...
    [RR,pp ] = corrcoef(aggSaccadeFreq,aggAvgSpeed);
    RAvgSpeed(ll) = RR(1,2);
    pAvgSpeed(ll) = pp(1,2);
    [RR,pp ] = corrcoef(aggSaccadeFreq,aggAvgTurnRate);
