...
%     fileName2 = ['..\..\data\', cell2mat(subject(ii)),'\',num2str(trialNum(j)),'\','eegTime.csv'];
%     if isfile(fileName1)
...
%         subtightplot(5,8,j+24,opt,opt,opt)
%         separation = 0; % variable to separate channel plots
%         for k = 1:size(frontalEEG,1)
%             plot(time,frontalEEG(k,:)+separation);
%             separation = separation + 150;
%             hold on
%         end
%         ylim([-100, 1200]);
%         xlim([0 30]);
