function [muF_theta, psd_theta, muF_alpha, psd_alpha, clu, clw]=eegAlphaPower(eeg_baseline, eeg_trial, dt,  wts, loadType,trantype, ...
    T, theta_r, alpha_r )
%
...
for cc=1:nchan                              % for each channel
    ...
    if(numel(freq_t(idx_t_alpha))>1)
        psd_alpha(cc,2)=interp1(freq_t(idx_t_alpha), psd_t(idx_t_alpha), muF_alpha(cc,2));
    elseif (numel(freq_t(idx_t_alpha)) == 1)
        psd_alpha(cc,2) = psd_t(idx_t_alpha);
    elseif (numel(freq_t(idx_t_alpha)) == 0)
        psd_alpha(cc,2) = 0;
    end
