...
for ii = 1:numel(subject)
    ...
    for j = 1:numel(trialNum)
        ...
        if isfile(fileName)
            ...
            % end
            c = 1;
            for mm = 1:samplerate*baselineWindow:numel(eeg(:,2))-samplerate*baselineWindow
                [~,~,~,~,~,cognitiveLoad(c)] = cogload(baselineEEG(:,2:end)', eeg(mm:mm+samplerate*baselineWindow-1,2:end)', ...
                    1/samplerate,eegChannelWeighting/sum(eegChannelWeighting),'alphaDiff','fft');
                c = c + 1;
            end
